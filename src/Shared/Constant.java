/**
 * @SER516 Project2_Team05
 */

package Shared;

import java.awt.Color;
import java.awt.Font;

/**
 * Constants class consists of common values used in all the classes of client and server
 **/
public class Constant {

	public static final int PORT_NUMBER = 1516;
	public static final String HOST_NAME = "localhost";
	public static final String CONSOLENAME = " Console :";
	public static final Font INPUTNUMBERFONT = new Font("Courier New", Font.BOLD, 20);
	public static final String STARTSTOPLABELNAME = "start / stop";
	public static final Color LIGHTBLUE = new Color(173,216,230);
	public static final Color LIGHTPINK = new Color(255,182,193);
	
}
